...
from agent_tasks.direct.wheel_leg_v1.wheel_leg_base.env_cfg import EventCfg, WheelLegFlatEnvCfg  # 父类：25 赛季 V3 的配置（大部分通用参数继承自它）
from agent_tasks.manager.mdp.terrain import TerrainCommandOverrideCfg  # "某地形上覆盖速度指令"的配置
from agent_tasks.direct.wheel_leg_v1.wheel_leg_task.cfg_utils import *  # V14 专用的常量/工具（观测裁剪表、重置姿势、课程默认参数等）
...
class WheelLegV1FlatEnvCfg(WheelLegFlatEnvCfg):
    ...
    # robot_cfg = WheelLegV1_CFG.replace(prim_path="/World/envs/env_.*/Robot").copy()  # （备选：二代无云台）
    ...
    simple_reset_root_height = 0.35       # 复位高度（米）：高于轮触地，留出轻微下落缓冲
    ...
    wheel_balance_sigma: float = 25.0
    ...
    # REWARD MAP V14_FLAT:
    # - 仅直接作用于 WheelLegV1FlatEnvCfg 及继承后未重写 rewards 的 flat 类任务。
    ...
    rewards = OrderedDict(          # ★权重表对齐 Wheelbipe-V14-Flat-v0（wheelbipe_V14/env_cfg.py:926）
        ...
        leg_joint_pair_pos_diff=0.0,  # Roll 修正需要允许左右腿产生差异
        roll_wheel_height=-2.0,      # 通过两条串联腿调节左右轮端高度，恢复 base_link Roll
        roll_recovery=1.5,           # Roll 误差下降奖励
        pitch_recovery=0.5,          # Pitch 误差下降奖励
        leg_action_l2=-0.002,         # 腿优先保持名义姿态，运动由轮子完成
        joint_torque=-1e-4,          # 力矩惩罚（省电+保护电机）
        wheel_balance_response=3.0,  # 俯仰误差由轮速及时接住
        wheel_roll_response=1.0,     # 前后/旋转指令由轮速完成
        wheel_hop=-5.0,              # V0 仅惩罚双轮同时离地
        wheel_acc=-2e-9,             # 轮加速度轻惩罚，避免压制滚动平衡
        wheel_vel=-1e-6,             # 轮速轻微正则，不能压制主动平衡
        wheel_power=-1e-5,           # 轮功率轻微正则，不能压制驱动和制动
...
@configclass
class WheelLegV1FlatEnvCfg_v2_Play(WheelLegV1FlatEnvCfg_v2):
...
''' 小陀螺训练 '''
@configclass
class WheelLegV1RoughEnvCfg(WheelLegV1FlatEnvCfg_v2):
# class WheelLegV1RoughEnvCfg(WheelLegV1FlatEnvCfg):
...
''' 跑场训练 '''
@configclass
class WheelLegV1RoughEnvCfg_v1(WheelLegV1FlatEnvCfg_v1):
...
@configclass
class WheelLegV1FlatEnvCfg_Play(WheelLegV1FlatEnvCfg):
    # 平地基础任务的演示配置
...
@configclass
class WheelLegV1FlatDreamWaqEnvCfg(WheelLegV1FlatEnvCfg):
...
@configclass
class WheelLegV1FlatDreamWaqEnvCfg_Play(WheelLegV1FlatEnvCfg_Play):
...
@configclass
class WheelLegV1FlatHIMEnvCfg(WheelLegV1FlatEnvCfg):
...
@configclass
class WheelLegV1FlatHIMEnvCfg_Play(WheelLegV1FlatEnvCfg_Play):
...
@configclass
class WheelLegV1FlatNP3OBarlowEnvCfg(WheelLegV1FlatEnvCfg):
...
@configclass
class WheelLegV1FlatNP3OBarlowEnvCfg_Play(WheelLegV1FlatEnvCfg_Play):
...
@configclass
class WheelLegV1RoughEnvCfg_Play(WheelLegV1RoughEnvCfg):
...
@configclass
class WheelLegV1RoughEnvCfg_v1_Play(WheelLegV1RoughEnvCfg_v1):
