...
# ─────────────────────────────────────────────────────────────────────────────
# 【文件总览·给初学者】本文件定义 V14 机器人的强化学习"环境类" WheelLegV1Env。
...
from agent_tasks.direct.wheel_leg_v1.wheel_leg_terrain.env import WheelLegTerrainEnv  # 父类：V13 环境，腿/轮控制与奖励公式等通用逻辑都在这里
from agent_tasks.direct.wheel_leg_v1.wheel_leg_task.cfg_utils import V14_ROUGH_HEIGHT_OFFSET_CURRICULUM_DEFAULT_CFG  # 粗糙地形"高度课程"的默认参数
from agent_tasks.direct.wheel_leg_v1.wheel_leg_task.env_cfg import WheelLegV1FlatEnvCfg  # 本环境配套的默认配置类（参数表）
...
class WheelLegV1Env(WheelLegTerrainEnv):
    """V14 机器人环境：在 V13 基础上增加云台控制、地形课程、轨迹录制。"""

    cfg: WheelLegV1FlatEnvCfg  # 类型注解：本环境持有的配置对象类型（参数表）

    def __init__(self, cfg: WheelLegV1FlatEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _validate_v14_bookkeeping(self) -> None:
        ...
        if expected_joint_name not in self.robot.joint_names:
            raise RuntimeError(f"WheelLeg V1 is missing required joint: {expected_joint_name}")
        if expected_body_name not in self.robot.body_names:
            raise RuntimeError(f"WheelLeg V1 is missing required body: {expected_body_name}")

        # Wheel_leg_V1 期望：4 腿关节 + 2 轮关节；无弹簧/云台/guide
        checks = {                                            # 名称 → (实际数量, 期望数量)
            "legs_act": (len(self._legs_act_idx), 4),         # L/R_joint1+joint2
            "wheel": (len(self._wheel_idx), 2),               # L/R_joint3
            "legs_inact": (len(self._legs_inact_idx), 0),     # 无被动关节
        ...
        if bad_checks:
            raise RuntimeError(
                "WheelLeg V1 asset discovery mismatch: " + ", ".join(bad_checks)  # 一次性报出所有问题
            )
