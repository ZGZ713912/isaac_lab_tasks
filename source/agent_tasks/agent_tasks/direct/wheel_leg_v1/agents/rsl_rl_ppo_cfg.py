...
class WheelLegTerrainFlatPPORunnerCfg(WheelLegBaseFlatPPORunnerCfg):
    ...
    algorithm = RslRlPpoAlgorithmCfg(  # 重写算法配置：价值损失权重 2.0→4.0
        ...
        schedule="adaptive",  # 对齐 wheelbipe V14 Flat：按 KL 自适应
        gamma=0.99,
        lam=0.95,
        desired_kl=0.01,
        max_grad_norm=1.0,
...
@configclass
class WheelLegV1FlatDreamWaqPPORunnerCfg(DreamWaqPPORunnerCfg):
    # V14 平地 DreamWaQ：继承 DreamWaQ 配置，只改实验名和迭代数
    experiment_name = "wheel_leg_v1_flat_dreamwaq_direct"
    max_iterations = 10000          # 迭代减半（该算法收敛更快）



@configclass
class WheelLegV1FlatHIMPPORunnerCfg(HIMPPORunnerCfg):
    # V14 平地 HIMLoco：同上
    experiment_name = "wheel_leg_v1_flat_him_direct"
    max_iterations = 5000           # HIM 迭代更少

@configclass
class WheelLegV1FlatNP3OBarlowPPORunnerCfg(NP3OPPORunnerCfg):
    # V14 平地 NP3O+BarlowTwins：显式重写一遍网络/算法字典（5 条安全约束版）
    experiment_name = "wheel_leg_v1_flat_np3o_barlow_direct"
    max_iterations = 3000
...
@configclass
class WheelLegV1RoughPPORunnerCfg(WheelLegTerrainRoughPPORunnerCfg):
