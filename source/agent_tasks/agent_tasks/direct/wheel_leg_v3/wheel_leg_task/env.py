...
# ─────────────────────────────────────────────────────────────────────────────
# 【文件总览·给初学者】本文件定义 V14 机器人的强化学习"环境类" WheelLegV3Env。
...
from agent_tasks.direct.wheel_leg_v3.wheel_leg_terrain.env import WheelLegTerrainEnv  # 父类：V13 环境，腿/轮控制与奖励公式等通用逻辑都在这里
from agent_tasks.direct.wheel_leg_v3.wheel_leg_task.cfg_utils import V14_ROUGH_HEIGHT_OFFSET_CURRICULUM_DEFAULT_CFG  # 粗糙地形"高度课程"的默认参数
from agent_tasks.direct.wheel_leg_v3.wheel_leg_task.env_cfg import WheelLegV3FlatEnvCfg  # 本环境配套的默认配置类（参数表）
...
class WheelLegV3Env(WheelLegTerrainEnv):
    """V14 机器人环境：在 V13 基础上增加云台控制、地形课程、轨迹录制。"""

    cfg: WheelLegV3FlatEnvCfg  # 类型注解：本环境持有的配置对象类型（参数表）

    def __init__(self, cfg: WheelLegV3FlatEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._undesired_contact_link_idx = self._find_contact_sensor_indices(  # "不希望接触"的部件索引（碰到就罚分）
            [
                "base_link",        # 车体
                "L_link1", "L_link2",   # 左右腿杆（Wheel_leg_V1：link1/2 为腿，link3 为轮）
                "R_link1", "R_link2",
            ]
        )
        self._desired_contact_link_idx = self._find_contact_sensor_indices(["(L_link3|R_link3)"])  # "希望接触"的部件：只有轮子贴地是正常的

        # —— 重置时检测"这些部位是否触地"的索引（触地说明机器人翻车/卡住）——
        reset_contact_body_names = ["base_link"]   # Wheel_leg_V1：车体触地即判定翻车/卡死
        terrain_cfg = getattr(self.cfg, "terrain", None)  # 看看任务用什么地形
        if getattr(terrain_cfg, "terrain_type", None) != "plane":
            # 粗糙地形上车体可能合法贴坡/蹭地：不再用触地判翻（姿态/高度终止兜底）
    ...
    def _reset_idx(self, env_ids):
        ...
        self._simple_root_reset(env_ids)                      # ③ Wheel_leg_V1：简单根状态复位（抬升→下落）
        self._reset_gimbal_joints(env_ids)                    # ④ V14 额外重置云台关节（本车为空，安全跳过）
        self._append_rough_height_offset_curriculum_log()     # ⑤ 课程状态写入日志

    def _simple_root_reset(self, env_ids):
        """Wheel_leg_V1 简单根状态复位。

        wheelbipe 的"预定义地面/空中姿态 + 五连杆 IK 摆腿"重置对本车不适用
        （无五连杆、无 IK 运动学），已用 use_leg_random_start=False 关闭该机制；
        这里只做：把车抬到 simple_reset_root_height 高度 + 随机 yaw + 速度清零，
    ...
    def _resolve_names_to_indices(
        ...
        *,
        ...
        if missing_names:
            raise RuntimeError(
                f"WheelLeg V14 {kind} discovery failed, missing names: {missing_names}"
            )
    ...
    def _validate_v14_bookkeeping(self) -> None:
        ...
        expected_joint_name = "L_joint1"                      # 抽查一个必须存在的关节
        expected_body_name = "L_link1"                        # 和一条必须存在的连杆
        if expected_joint_name not in self.robot.joint_names:
            raise RuntimeError(f"WheelLeg V3 is missing required joint: {expected_joint_name}")
        if expected_body_name not in self.robot.body_names:
            raise RuntimeError(f"WheelLeg V3 is missing required body: {expected_body_name}")

        # Wheel_leg_V3 期望：4 腿电机 + 2 轮电机 + 12 被动闭链关节；无弹簧/云台/guide
        checks = {                                            # 名称 → (实际数量, 期望数量)
            "legs_act": (len(self._legs_act_idx), 4),         # L_joint1/LL_joint1/R_joint1/RR_joint1
            "wheel": (len(self._wheel_idx), 2),               # L/R_joint3
            "legs_inact": (len(self._legs_inact_idx), 12),    # 四杆 + 气簧被动关节
            "ordered_leg_joint_idx": (len(self._ordered_leg_joint_idx), 4),
            "ordered_leg_body_idx": (len(self._ordered_leg_body_idx), 4),
        }
        bad_checks = [
        ...
        if bad_checks:
            raise RuntimeError(
                "WheelLeg V3 asset discovery mismatch: " + ", ".join(bad_checks)  # 一次性报出所有问题
            )
