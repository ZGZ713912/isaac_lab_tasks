...
from agent_world.assets.wheel_leg_V3 import WheelLegV3_CFG  # Wheel_leg_V1 资产（legs_act=J1/J2×2 + wheel=J3×2）

def _actuator_joint_names_expr(key: str) -> list:
    ...
    act = WheelLegV3_CFG.actuators.get(key)
    return list(act.joint_names_expr) if act is not None else []
from agent_tasks.direct.wheel_leg_v3.wheel_leg_base.env_cfg import EventCfg, WheelLegFlatEnvCfg  # 父类：25 赛季 V3 的配置（大部分通用参数继承自它）
from agent_tasks.manager.mdp.terrain import TerrainCommandOverrideCfg  # "某地形上覆盖速度指令"的配置
from agent_tasks.direct.wheel_leg_v3.wheel_leg_task.cfg_utils import *  # V14 专用的常量/工具（观测裁剪表、重置姿势、课程默认参数等）


@configclass
class EventCfgV14(EventCfg):
    """Event configuration for the WheelLeg V14 direct RL environments."""
    ...
    #     mode="startup",
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", body_names=["L_link1", "L_link1"]),
    ...
    add_leg_mass = EventTerm(         # 腿杆质量随机化：×0.9~1.1（Wheel_leg_V1：左右腿杆 link1/link2）
        ...
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg(   # 覆盖所有腿连杆（Wheel_leg_V1 无前腿/后腿/弹簧/云台）
                "robot",
                body_names=["L_link1", "L_link2", "R_link1", "R_link2"],
            ),
            "mass_distribution_params": (0.9, 1.1),
            "operation": "scale",
        },
    ...
    guide_material = None             # Wheel_leg_V1 无 guide 部件：禁用（占位 L_joint1_never 会在 SceneEntityCfg.resolve 空匹配报错）
    ...
    #         # "armatuL_link3 material=static=1.1905 dynamic=0.9076 restitution=0.1467 contact=|F|=10.43 peak=33.36re_distribution_params": (0.001, 0.003),
    #         "operation": "add",
    ...
    leg_front_joint_frictions = EventTerm(  # 前腿主动关节摩擦随机化：在默认值上"加"随机量
        ...
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=["(L_joint1|R_joint1)"]),  # 4 个前腿髋关节
    ...
    leg_rear_joint_frictions = EventTerm(   # 后腿主动关节摩擦随机化（参数同前腿）
        ...
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=["(L_joint2|R_joint2)"]),  # 4 个后腿髋关节
    ...
    legs_inact_joint_frictions = None  # Wheel_leg_V1 无被动腿关节：禁用（原 5×L_joint1_never 占位会空匹配报错）
    ...
    #     },
    # )
    gimbal_joint_frictions = None      # Wheel_leg_V1 无云台关节：禁用（原 L_joint1_never 占位会空匹配报错）
    ...
    spring_damping = None              # Wheel_leg_V1 无弹簧关节（use_spring=False）：禁用（原 L_joint1_never 占位会空匹配报错）
    leg_effort_noise = EventTerm(     # 腿电机输出力矩扰动（默认范围不改变行为，训练时可放开）
        func=mdp.randomize_actuator_effort_output,
        min_step_count_between_reset=720,
        mode="reset",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=["(L_joint1|R_joint1)","(L_joint2|R_joint2)"]),  # 8 个腿电机
    ...
    wheel_effort_noise = EventTerm(   # 轮电机输出力矩扰动（同上）
        ...
        mode="reset",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names="(L_joint3|R_joint3)"),  # 2 个轮电机
            # 输出力矩扰动：tau = clip(tau_nominal * scale + bias + N(0, noise_std))
            # 默认配置不改变行为，需要训练时可把 scale/bias/noise_std 的范围放开。
...
class EventCfgV14_Play(EventCfgV14):
    """Play-mode event configuration for WheelLeg V14."""
...
class CurriculumCfgV14:
    """WheelLeg V14 command curriculum configuration."""
...
# ─────────────────────────────────────────────────────────────────────────────
@configclass
class WheelLegV3FlatEnvCfg(WheelLegFlatEnvCfg):
    ...
    robot_cfg: ArticulationCfg = WheelLegV3_CFG.replace(prim_path="/World/envs/env_.*/Robot").copy()  # ★用哪台机器人：V14 二代，挂到每个环境自己的路径下
    # robot_cfg: ArticulationCfg = WheelLegV3_CFG.replace(prim_path="/World/envs/env_.*/Robot").copy()  # （备选：无云台版）
    # robot_cfg = WheelLegV3_CFG.replace(prim_path="/World/envs/env_.*/Robot").copy()  # （备选：二代无云台）

    # —— 从执行器配置里取出各组关节名的正则表达式（父类靠它们识别关节用途）——
    legs_act_name = robot_cfg.actuators["legs_act"].joint_names_expr   # 主动腿关节（4 个髋电机）
    legs_inact_name = _actuator_joint_names_expr("legs_inact")  # 从动腿关节（Wheel_leg_V1 无 → []）
    wheel_name = robot_cfg.actuators["wheel"].joint_names_expr         # 轮电机（2 个）
    spring_name = _actuator_joint_names_expr("spring")       # 弹簧关节（Wheel_leg_V1 无 → []）
    gimbal_yaw_name = _actuator_joint_names_expr("gimbal_yaw")    # 云台 yaw 关节（无 → []）
    gimbal_pitch_name = _actuator_joint_names_expr("gimbal_pitch")  # 云台 pitch 关节（无 → []）
    use_gimbal = False                # Wheel_leg_V1 无云台
    ...
    # ★临时 joint1 训练限位：V2 闭链髋关节限位未标定，先关闭（标定后填 joint1_pos_limit_range 并置 True）
    use_joint1_pos_limit = False
    joint1_pos_limit_range = (-0.2, 1.0)  # rad, (lower, upper)

    # ★ 身高指令：V2 零位（q=0）站高 ≈0.231m，可达区间约 [0.20, 0.25] m（见 wheel_leg_V2 合同）。
    default_height_cmd = 0.23        # 默认身高指令 0.23 m（URDF 零位静平衡高）

    # —— 观测延迟模拟（sim2real：真机传感/通信有延迟，训练时就让策略适应）——
    # 每项 = [最小延迟步数, 最大延迟步数]，重置时在区间内随机取固定值
    obs_delay_cfg = {
        "root_ang_vel_b": [2, 8],        # 角速度观测延迟（100Hz 下 20~80ms，物理时长同 50Hz 的 1~4 步）
        "projected_gravity_b": [2, 8],   # 重力方向(姿态)延迟
        "joint_pos": [2, 8],             # 关节角延迟
        "joint_vel": [2, 8],             # 关节速度延迟
    }
    # obs_delay_cfg = {
    #     "root_ang_vel_b": [2, 5],
    ...
    # —— 动作延迟模拟（真机电机响应滞后）——
    act_delay_cfg = {
        "leg_actions": [2, 6],           # 腿动作延迟（100Hz 下 20~60ms，物理时长同 50Hz 的 1~3 步）
        "wheel_actions": [2, 6],         # 轮动作延迟
    }
    # act_delay_cfg = {
    #     "leg_actions": [1, 4],
    #     "wheel_actions": [1, 4],
    ...
    height_range = [0.20, 0.25]      # V2 闭链可达身高区间（与 wheel_leg_V2 合同一致）
    ...
    alpha_offset = [-6.61/180.*torch.pi,  # 各连杆的安装角偏移(弧度)
                    ...
                    (29.7)/180.*torch.pi,
                    29.7/180.*torch.pi]
    leg_length_range = [0.13,0.32]        # 腿长采样范围(米)（IK 腿摆放已关闭，仅作记录）
    leg_angle_range = [-0.5*torch.pi,0.75*torch.pi]  # 腿摆角采样范围（同上）
    use_leg_random_start = False          # Wheel_leg_V1：无五连杆 IK 运动学，关闭"随机腿长/腿角+IK 摆腿"
    use_predefined_leg_random_start = False   # 预定义腿部姿势重置随之关闭
    predefined_reset_ground = copy.deepcopy(V14_PREDEFINED_RESET_GROUND)  # 保留字段（不再使用）
    use_simple_root_reset = True          # 改用简单根状态复位：抬到 simple_reset_root_height 后自然下落
    simple_reset_root_height = 0.30       # 复位高度（米）：高于轮触地，留出轻微下落缓冲（V2 零位 0.23）
    enable_state_machines = False         # 状态机总开关（腾空/跳台阶等，V14 基础版关闭）
    termination_duration_enabled = True   # 终止判定需要"连续多步满足"才生效
    termination_duration_steps = 40       # 连续 40 步(0.4s @100Hz)都异常才算摔倒（防瞬时误判）
    ...
    # REWARD MAP V14_FLAT:
    # - 仅直接作用于 WheelLegV3FlatEnvCfg 及继承后未重写 rewards 的 flat 类任务。
    ...
    # 奖励权重：严格采用 wheelbipe_V14 的 Flat 权重表（如需 V3 专用调参请在此覆盖）。
    rewards = OrderedDict(
    ...
    def __post_init__(self):
        ...
        self.decimation = 2              # 每个策略步内跑 2 次物理仿真（100Hz）
        self.sim.dt = 1 / 200.0          # 物理仿真步长 1/200 秒 → 策略频率 = 200/2 = 100Hz
...
# ─────────────────────────────────────────────────────────────────────────────
@configclass
class WheelLegV3FlatEnvCfg_v2(WheelLegV3FlatEnvCfg):
...
@configclass
class WheelLegV3FlatEnvCfg_v2_Play(WheelLegV3FlatEnvCfg_v2):
...
''' 腾空落地预训练 '''
@configclass
class WheelLegV3FlatEnvCfg_v1(WheelLegV3FlatEnvCfg):
...
''' 小陀螺训练 '''
@configclass
class WheelLegV3RoughEnvCfg(WheelLegV3FlatEnvCfg_v2):
# class WheelLegV3RoughEnvCfg(WheelLegV3FlatEnvCfg):
...
''' 跑场训练 '''
@configclass
class WheelLegV3RoughEnvCfg_v1(WheelLegV3FlatEnvCfg_v1):
...
@configclass
class WheelLegV3FlatEnvCfg_Play(WheelLegV3FlatEnvCfg):
    # 平地基础任务的演示配置
...
@configclass
class WheelLegV3FlatDreamWaqEnvCfg(WheelLegV3FlatEnvCfg):
...
@configclass
class WheelLegV3FlatDreamWaqEnvCfg_Play(WheelLegV3FlatEnvCfg_Play):
...
@configclass
class WheelLegV3FlatHIMEnvCfg(WheelLegV3FlatEnvCfg):
...
@configclass
class WheelLegV3FlatHIMEnvCfg_Play(WheelLegV3FlatEnvCfg_Play):
...
@configclass
class WheelLegV3FlatNP3OBarlowEnvCfg(WheelLegV3FlatEnvCfg):
...
@configclass
class WheelLegV3FlatNP3OBarlowEnvCfg_Play(WheelLegV3FlatEnvCfg_Play):
...
@configclass
class WheelLegV3RoughEnvCfg_Play(WheelLegV3RoughEnvCfg):
...
@configclass
class WheelLegV3RoughEnvCfg_v1_Play(WheelLegV3RoughEnvCfg_v1):
