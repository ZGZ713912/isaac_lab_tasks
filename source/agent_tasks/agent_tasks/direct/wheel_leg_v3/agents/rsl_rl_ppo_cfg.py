...
@configclass
class WheelLegBaseFlatPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    ...
    experiment_name = "wheel_leg_base_flat_direct"  # 实验名（决定 logs/ 下的目录名）
...
@configclass
class WheelLegTerrainFlatPPORunnerCfg(WheelLegBaseFlatPPORunnerCfg):
    # V13 平地任务：继承 25v3 配置，只改实验名和价值损失权重
    experiment_name = "wheel_leg_v3_flat_direct"
...
@configclass
class WheelLegTerrainRoughPPORunnerCfg(WheelLegBaseFlatPPORunnerCfg):
    # V13 粗糙地形任务：完全继承 25v3 配置，只改实验名
    experiment_name = "wheel_leg_v3_rough_direct"

@configclass
class WheelLegV3FlatPPORunnerCfg(WheelLegTerrainFlatPPORunnerCfg):
...
@configclass
class WheelLegV3FlatDreamWaqPPORunnerCfg(DreamWaqPPORunnerCfg):
    # V14 平地 DreamWaQ：继承 DreamWaQ 配置，只改实验名和迭代数
    experiment_name = "wheel_leg_v3_flat_dreamwaq_direct"
    max_iterations = 10000          # 迭代减半（该算法收敛更快）



@configclass
class WheelLegV3FlatHIMPPORunnerCfg(HIMPPORunnerCfg):
    # V14 平地 HIMLoco：同上
    experiment_name = "wheel_leg_v3_flat_him_direct"
    max_iterations = 5000           # HIM 迭代更少

@configclass
class WheelLegV3FlatNP3OBarlowPPORunnerCfg(NP3OPPORunnerCfg):
    # V14 平地 NP3O+BarlowTwins：显式重写一遍网络/算法字典（5 条安全约束版）
    experiment_name = "wheel_leg_v3_flat_np3o_barlow_direct"
    max_iterations = 3000
...
@configclass
class WheelLegV3RoughPPORunnerCfg(WheelLegTerrainRoughPPORunnerCfg):
