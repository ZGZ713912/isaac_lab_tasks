...
from agent_tasks.direct.wheel_leg_v3.wheel_leg_base.env_cfg import WheelLegBaseFlatEnvCfg, WheelLegFlatEnvCfg
from agent_tasks.direct.wheel_leg_v3.state_machines import WheelLegStateMachineManager
...
class WheelLegBaseEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: WheelLegFlatEnvCfg | WheelLegBaseFlatEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._wheel_idx, self._wheel_idx_name =  self.robot.find_joints(self.cfg.wheel_name)
        ...
        # 轮关节轴左右镜像符号（L=+1, R=-1）：policy 动作正 = 车轮正向滚动 = 车体前进。
        # 优先按关节名判定（不依赖 find_joints 返回顺序），退回 cfg.wheel_joint_sign 的位置约定。
        wheel_sign_cfg = getattr(self.cfg, "wheel_joint_sign", None)
        name_sign: list[float] | None = []
        for wheel_name in list(self._wheel_idx_name):
            if wheel_name.startswith("L"):
                name_sign.append(1.0)
            elif wheel_name.startswith("R"):
                name_sign.append(-1.0)
            else:
                name_sign = None
                break
        if name_sign is not None and len(name_sign) == len(self._wheel_idx):
            sign_values = name_sign
        elif wheel_sign_cfg is not None and len(wheel_sign_cfg) == len(self._wheel_idx):
            sign_values = [float(v) for v in wheel_sign_cfg]
        else:
            sign_values = [1.0] * len(self._wheel_idx)
        self._wheel_joint_sign = torch.tensor(
            sign_values, dtype=torch.float, device=self.device
        ).reshape(1, -1)

        # —— 闭链 + 气簧（Wheel_leg_V2 机器人）——
        # 气簧是 excludeFromArticulation 的 loop prismatic，不能下发 effort；改为按 BKB
        # 力曲线对两端刚体施加轴向力（几何来自 constraints.json）。
        self._constraints_path = getattr(self.cfg, "constraints_path", None) or os.path.join(
            AssetPath, "usd_files", "Wheel_leg_V2", "constraints.json"
        )
        self.constraints = {}
        if os.path.isfile(self._constraints_path):
            with open(self._constraints_path, "r", encoding="utf-8") as handle:
                self.constraints = json.load(handle)
        else:
            print(f"[WheelLegV3] constraints file not found: {self._constraints_path}")
        ...
        self._closure_body_ids = None
        self._closure_body_names = None
        if self._gas_spring_enabled:
            self._setup_gas_springs()
        self._setup_closure_watchdog()
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self.wheel_actions = self.wheel_action_scale * wheel_policy_actions * self._wheel_joint_sign
    ...
    def _apply_action(self) -> None:
        ...
        # 闭链气簧：每物理子步施加轴向力（loop prismatic 无法下发 effort）
        if self._gas_spring_enabled:
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # 闭链看门狗：任一闭合点误差超阈值 → 立即安全 reset（不计入延迟窗口）
        closure_bad = self._get_closure_bad()
        if closure_bad is not None:
            terminate |= closure_bad
            self.extras.setdefault("log", {})["Termination/closure"] = closure_bad.float().mean()
    ...
    # ------------------------------------------------ closed chain / gas spring
    def _setup_gas_springs(self) -> None:
        """按 constraints.json 的 gas_springs 锚点建立气簧力控张量。"""
        ...
        for name in spring_body_names:
            if body_names.count(name) != 1:
                raise RuntimeError(f"[WheelLegV3] gas-spring body not found/ambiguous: {name}")
        self._gas_spring_body_ids = torch.tensor(
            [body_names.index(name) for name in spring_body_names],
    ...
    def _apply_gas_spring_forces(self) -> None:
        """按气簧力曲线对两杆施加轴向力（世界力→各刚体局部系，作用在各杆销点）。"""
    ...
    def _setup_closure_watchdog(self) -> None:
        """建立闭链看门狗所需的 body 索引（球约束 + 气簧棱柱约束两端）。"""
        closures = list(self.constraints.get("closures", []))
        if not closures:
            self._closure_body_ids = None
            self._closure_body_names = None
            return
        ...
        for closure in closures:
            for key in ("body0", "body1"):
                ...
                if name in names:
                    continue
                if body_names.count(name) != 1:
                    raise RuntimeError(f"[WheelLegV3] closure body not found/ambiguous: {name}")
                names.append(name)
                ids.append(body_names.index(name))
        self._closure_body_names = names
        self._closure_body_ids = torch.tensor(ids, dtype=torch.long, device=self.device)

    def _closure_errors(self) -> dict[str, torch.Tensor]:
        """每个闭合点两杆世界点距离（米），对标 validate_wheel_leg_v2_closed.py。"""
        errors: dict[str, torch.Tensor] = {}
        if self._closure_body_ids is None:
            return errors
        from isaaclab.utils.math import quat_apply

        body_pos = self.robot.data.body_pos_w
        body_quat = self.robot.data.body_quat_w
        for closure in self.constraints.get("closures", []):
            i0 = self._closure_body_ids[self._closure_body_names.index(closure["body0"])]
            i1 = self._closure_body_ids[self._closure_body_names.index(closure["body1"])]
    ...
    def _get_closure_bad(self) -> torch.Tensor | None:
        """闭链发散安全掩码：任一闭合点误差超阈值即终止；同时写入 TB 指标。"""
        errors = self._closure_errors()
        if not errors:
            return None
        closure_max = torch.stack(tuple(errors.values()), dim=-1).amax(dim=-1)
        tol = float(getattr(self.cfg, "closure_error_tolerance", 0.0))
