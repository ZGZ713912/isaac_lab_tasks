...
class WheelLegBaseEnvCfg(DirectRLEnvCfg):
    ''' env configs '''
    # env
    decimation = 2 # 1/200 -> 2；策略频率 = 200/2 = 100Hz（Wheel_leg_V3 目标控制频率）
    ...
    use_spring: bool = False          # Wheel_leg_V3 无虚拟弹簧：气簧走 loop-prismatic 手动力控（见 gas_spring_*）
    # —— 闭链 + 气簧（Wheel_leg_V2 机器人专用，USD 内为 4 spherical + 2 prismatic 气簧）——
    # 气簧为 excludeFromArticulation 的 loop prismatic，无法下发 effort，由环境按 BKB 力曲线
    # 对两端刚体施加轴向力；几何来自 constraints.json。
    ...
    # 轮关节轴左右镜像（L_joint3=+Y, R_joint3=-Y）：动作正=前进，故右轮取负号。
    wheel_joint_sign: list[float] = field(default_factory=lambda: [1.0, -1.0])
...
class WheelLegBaseFlatEnvCfg(WheelLegBaseEnvCfg):
    ...
    # robot
    robot_cfg: ArticulationCfg = WheelLegV3_CFG.replace(prim_path="/World/envs/env_.*/Robot")
    ...
    # height commands
    default_height_cmd = 0.23      # Wheel_leg_V3：URDF 零位静平衡高 ≈0.231m，与 V2 合同一致
    height_range = [0.20, 0.25]    # V2 闭链可达身高区间（见 wheel_leg_V2 合同 commands）
    ...
    # 未列出的关节沿用上面的默认 ±3.14。Wheel_leg_V3 (V2 闭链) URDF 全 continuous，
    # 暂不设机械限位；标定后按 "LL_joint1"/"RR_joint1" 填入即可生效。
    ...
    use_leg_random_start = False     # Wheel_leg_V3 闭链：不用五连杆 IK 摆腿，改 task 层简单复位
